...
def remove_from_exploration(command: Command, partyId: str) -> List[GamestateChange]:
    """
    ...
    """
    ...
    change = GamestateChange("", GamestateChange.Type.REMOVED_FROM_EXPLORATION.value)  

    # Execute command
    for instance_id in command.targetIds:
        removed = gamestate_controller.phase_exploration_controller.remove_from_exploration(gamestate, instance_id)
        if removed:
